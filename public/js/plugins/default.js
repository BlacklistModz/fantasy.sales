...
(function( $, window, document, undefined ) {
    ...
    var Upload = {
		...
		setBox: function () {
			...
			/*<h3>Start Adding Files to "test". It\'s Easy!</h3><div>Drag and drop them here, or click Upload Images.<br>Your images will also appear in your Site Media folder, so they’re easy to find and use. */
			$body.append( $main  );
    ...
    var ImageCover2 = {
		...
		setControl: function () {
			...
			var $edit = $('<div/>',{ class:'image-cover-edit', text: 'เปลี่ยนรูป'});
			var $del = $('<div/>',{ class:'image-cover-edit', text: 'ลบ'});
	...
	var CloseDate = {
		init: function (options, elem) {
			...
			if( !self.options.lang ){
				self.options.lang = $('html').attr('lang') || 'th';
			}
		...
		setElem: function () {
			...
			var $table = $('<table/>', {class: 'calendarCloseDateGridTable'}).append( 
				...
				, $('<tr>').append( $('<td>', {colspan: 3, class: 'tar ptm'}).append(
					  self.$preveiw
					, $('<a>', {class: 'btn btn-cancel', text: textbtn[self.options.lang][0] })
					, $('<a>', {class: 'btn btn-blue btn-submit', text: textbtn[self.options.lang][1] })
				) )
			);
		...
		setTextCalendar: function() {
			...
			var TO = '-'; //self.options.lang == 'th' ? ' ถึง ' : ' to ';
			var $text = $('<span>');
		...
		setCalendar: function( date ){
			...
			var endYear = self.options.lastYear || 2000;//self.options.firstYear || self.today.getFullYear()-5;
						...
						onSubmit: function ($d) {
							...
							Event.inlineSubmit( $form ).done(function( result ) {
								...
								if( result.data ){
									...
									console.log( self.$elem.find('option') );
									self.$elem.find('option').last().before( $('<option>', {
										value: result.data.id,
										text: result.data.name
									}) );

									self.$elem.val( result.data.id );
									self.currVal = result.data.id;
								}
							});
						}
					});
	...
	var AddInput = {
		init: function (options, elem) {
			...
			// data-action
			self.options = $.extend( {}, $.fn.addinput.options, options );
			self.$input = $(elem);
			self.$elem = $(elem);
			...
			var placeholder = $('<div/>', {class: 'ui-box-input'});
			self.$elem.replaceWith(placeholder);
			self.$elem = placeholder;
			self.currID = self.options.id || null;

			self.$preview = $('<div>', {class:'preview'});
			self.changePreview();
			...
			self.$input.blur(function () {
				...
				if( !self.is_focus ){
					...
					if( self.currID ){
						self.$menu.find('[data-id='+ self.currID +']').addClass('active').siblings().removeClass('active');
						self.active();
					}
					else{
						self.hide();
				...
				if( e.which == 38 || e.which==40 ){
					...
					// var prev = e.which==38 ? -1:1;
					var li = self.$menu.find('li.active');
					if( e.which==38 ){
						prev = li.prev().length ? li.prev() : self.$menu.find('li:last-child');
						prev.addClass('active').siblings().removeClass('active');
					}
					else{
						next = li.next().length ? li.next() : self.$menu.find('li:first-child');
		...
		search: function () {
			...
			self.currVal = $.trim( self.$input.val() );
			if( self.currVal=='' ){
				/*self.hide();
				self.has_load = false;
				return false;*/
			...
			self.$menu.find('.uiLayerItem').remove();
			self.$menu.toggleClass('has-create', self.currVal!='');
			...
			self.$menu.delegate('li','click',function () {
				self.active();
			});
			...
			if( self.url ){
				$.get(self.url, {q: self.currVal, limit: self.options.limit}, function (res) {
					self.has_load = false;
				...
				self.has_load = false;
				$.each( self.options.options, function (index, obj) {
					self.setItemMenu( obj );
				} );
			}
		},
		setItemMenu: function ( obj ) {
			var self = this;
			...
			var $item = $('<li>', {class: 'uiLayerItem', 'data-id':obj.id}).html( __ui.anchorBucketed( obj ) )
			$item.data( obj );

			if( self.currVal!='' ){
				self.$create.before( $item );
			}else{
		...
		setCurr: function () {
			...
			if( self.currID ){
				self.$menu.find('[data-id='+ self.currID +']').addClass('active').siblings().removeClass('active');
			}else if( self.$menu.find('[data-id]').length>0 ){
				self.$menu.find('li').first().addClass('active').siblings().removeClass('active');
			}
		},
		...
		setMenu: function () {
			...
			self.$menu = $('<ul/>', {class: 'uiLayerBox'});
			// self.$menu = $('<ul/>', {class: 'ui-list ui-list-menu', role: "listbox"});
			...
			uiLayer.get(settings, self.$menu);
			self.$layer = self.$menu.closest('.uiLayer');

			self.$menu.css({
				minWidth: self.$input.outerWidth()
			});
			self.is_open = true;
		},
		...
		active: function () {
			...
			var data = self.$menu.find('.active').data();
			if( data.text ){
				self.currVal = data.text;
				self.currID = data.id;
			}
			else{
				self.currID = null;
			...
			self.$elem.addClass('has-preview');
			self.$preview.empty().append( self.currVal );

			self.changePreview();

			self.$input.val('').attr('placeholder', '').prop('disabled', true);
			self.hide();
		},
